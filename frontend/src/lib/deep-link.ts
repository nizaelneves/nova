/**
 * deep-link.ts — Handler for nova:// deep links in the Tauri desktop app.
 *
 ...
 *   nova://research/{session_id}   → open a research session
 *   nova://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `nova://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `nova://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `nova:` scheme.
 *
 * @example
 * parseDeepLink("nova://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("nova://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "nova:") return null;

    // The URL constructor treats "nova://research/abc123" such that
