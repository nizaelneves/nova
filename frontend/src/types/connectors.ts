...
export const SOURCE_CATALOG: ConnectorMeta[] = [
  ...
  // ── Communication ──────────────────────────────────────────────────
  {
    ...
    steps: [
      {
        label: 'Make sure 2-Step Verification is enabled, then generate a 16-character App Password (Mail / Other / "Nova"). Paste it below \u2014 spaces are fine, and use the app password, not your regular Gmail password.',
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to api.slack.com/apps and click "Create New App" → choose "From scratch". Name it "Nova" and pick your workspace',
      ...
      {
        label: '(Optional) Set the app icon: in the left sidebar click "Basic Information" → scroll to "Display Information" → upload the Nova logo',
        url: 'https://github.com/nizaelneves/nova/blob/main/assets/nova-slack-icon.jpg',
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to notion.so/profile/integrations → click "+ New integration". Name it "Nova", select your workspace, and click Submit',
      ...
      {
        label: 'To share ALL your pages at once: open any top-level page → click "..." (top right) → "Connections" → "Add connections" → search "Nova" → click it. This shares the page and all its sub-pages. Repeat for each top-level page, or share your entire workspace by doing this on every root page',
      },
      {
  ...
  },
  {
    ...
    steps: [
      ...
      {
        label: 'Click the "+" button at the bottom of the list. Navigate to Applications → Utilities → select "Terminal.app" (or iTerm2/Warp if you use those). If you\'re using the desktop app, also add "Nova.app" from Applications',
      },
      {
        label: 'Toggle the switch ON next to each app you added. Close and reopen your terminal (or restart Nova). iMessage data will be detected automatically — no credentials needed',
      },
    ],
  },
  // ── Documents ──────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      {
        label: 'Paste the full path below. Nova will index all .md files in the vault',
      },
    ],
    inputFields: [
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to Google Cloud Console → create a new project (or select an existing one). Give it any name (e.g. "Nova")',
  ...
  },
  {
    ...
    steps: [
      ...
      {
        label: 'Toggle the switch ON next to each app you added. Close and reopen your terminal (or restart Nova). Apple Notes will be detected automatically — no credentials needed',
      },
    ],
  },
  {
    ...
    steps: [
      ...
      {
        label: 'Toggle the switch ON next to each app you added. Close and reopen your terminal (or restart Nova). Apple Contacts will be detected automatically — no credentials needed',
      },
    ],
  },
  {
    ...
    steps: [
      {
        label: 'Go to the Azure Portal → App Registrations → click "+ New registration". Name it "Nova", select "Accounts in this organizational directory only", and click Register',
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to the Dropbox App Console and click "Create app". Choose "Scoped access" → "Full Dropbox" → give it a name (e.g. "Nova") → click "Create app"',
