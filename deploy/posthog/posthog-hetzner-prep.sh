...
# Nova's self-hosted PostHog analytics backend.
#
...
#
# Usage:
#   sudo bash posthog-hetzner-prep.sh analytics.nova.ai you@nova.ai
#
# After it finishes:
#   1. Visit https://<DOMAIN>/ and create the admin account.
#   2. Create project "Nova".
#   3. Settings → Project → grab the Project API Key (phc_…).
#   4. Update src/nova/core/config.py AnalyticsConfig defaults:
#        host = "https://<DOMAIN>"
#        key  = "phc_<new>"
...
Examples:
  sudo bash posthog-hetzner-prep.sh analytics.nova.ai team@nova.ai
...
Next steps:
  ...
  3. Create project "Nova".
  4. Settings → Project → Project API Key — copy the phc_… value.
  5. Update src/nova/core/config.py AnalyticsConfig defaults:
        host = "https://$DOMAIN"
  ...
  8. Ship a release of Nova with the new config defaults.
