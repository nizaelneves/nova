<#
.SYNOPSIS
    Register / unregister the Nova Windows scheduled task.

.DESCRIPTION
    The Windows equivalent of deploy/systemd/nova.service and
    deploy/launchd/com.nova.plist.

    Registers a per-user scheduled task named "Nova" that starts
    ...
    Arguments (install only):
      -InstallRoot <path>  default: %LOCALAPPDATA%\Nova (matches
                           install.ps1's default)
      -ListenHost <addr>   default: 127.0.0.1 (loopback). Set to 0.0.0.0
                           ONLY if you also set $env:NOVA_API_KEY
...
function Get-DefaultInstallRoot {
    ...
    if ($env:NOVA_HOME) { return $env:NOVA_HOME }
    return (Join-Path $env:LOCALAPPDATA 'Nova')
}
...
function Install-Task {
    ...
    if (-not (Test-Path $srcDir)) {
        Write-Fail "Nova source not found at $srcDir. Run install.ps1 first."
    }
    ...
    # Mirrors deploy/systemd/nova.service's EnvironmentFile guard.
    $isLoopback = ($ListenHost -eq '127.0.0.1' -or $ListenHost -eq 'localhost')
    if (-not $isLoopback -and -not $env:NOVA_API_KEY) {
        Write-Fail @"
ListenHost is $ListenHost (non-loopback) but `$env:NOVA_API_KEY is
...
and would also create a security hole. Set the env var first:
    ...
    `$env:NOVA_API_KEY = (uv run nova auth generate-key)
...
"@
    ...
    if (-not $isLoopback) {
        Write-Info "Persisting NOVA_API_KEY to User environment so the scheduled task can read it at logon."
    ...
    Register-ScheduledTask `
        ...
        -Settings $settings `
        -Principal $principal `
        -Description 'Nova API server (loopback default — see deploy/windows/README.md)' | Out-Null
