...
5. Clones the Nova repository to `%LOCALAPPDATA%\Nova`
...
The available env vars: `NOVA_SKIP_SERVICE`, `NOVA_SERVICE`,
`NOVA_FORCE`. If you need richer control, save the script first
...
```powershell
$srv = "$env:LOCALAPPDATA\Nova\src\deploy\windows\nova-service.ps1"
...
`$env:NOVA_API_KEY` is unset — same guard as the systemd unit's
...
|---------|---------|---------|---------|
| Service definition | `deploy/systemd/nova.service` | `deploy/launchd/com.nova.plist` | `deploy/windows/nova-service.ps1` (cmdlet-driven) |
...
```powershell
powershell -ExecutionPolicy Bypass -File "$env:LOCALAPPDATA\Nova\src\deploy\windows\nova-service.ps1" uninstall
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\Nova"
```
