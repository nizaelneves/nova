...
def test_tts_tool_execute(tmp_path):
    ...
    with patch("nova.tools.text_to_speech.TTSRegistry") as mock_registry:
...
def test_tts_omits_unset_voice_id_and_speed(tmp_path):
    ...
    with (
        patch("nova.tools.text_to_speech.TTSRegistry", registry),
        patch(
            "nova.core.config.load_config",
...
def test_tts_forwards_explicit_voice_id_and_speed(tmp_path):
    ...
    with patch("nova.tools.text_to_speech.TTSRegistry", registry):
        tool.execute(
            text="hello",
            backend="kokoro",
            voice_id="bf_emma",
