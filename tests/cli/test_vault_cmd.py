...
class TestVaultCmd:
    ...
    def test_vault_list_empty(self) -> None:
        with mock.patch(
            "nova.cli.vault_cmd._VAULT_FILE",
    ...
    def test_vault_roundtrip(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch("nova.cli.vault_cmd._VAULT_FILE", vault_file),
            mock.patch("nova.cli.vault_cmd._VAULT_KEY_FILE", key_file),
            mock.patch(
                "nova.cli.vault_cmd.DEFAULT_CONFIG_DIR",
                tmp_path,
            ),
        ):
            runner = CliRunner()
