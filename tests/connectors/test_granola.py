...
def connector(tmp_path: Path):
    ...
    from nova.connectors.granola import GranolaConnector  # noqa: PLC0415
...
@patch("nova.connectors.granola._granola_api_list_notes")
@patch("nova.connectors.granola._granola_api_get_note")
...
def test_format_note_content() -> None:
    ...
    from nova.connectors.granola import _format_note_content  # noqa: PLC0415
...
def test_sync_logs_note_count(
    ...
    with caplog.at_level(logging.INFO, logger="nova.connectors.granola"):
        list(connector.sync())
...
def test_end_to_end_ingest_and_search(
    ...
    from nova.agents.research_loop import (  # noqa: PLC0415
        _hit_url,
...
def test_validate_key_empty_raises() -> None:
    ...
    from nova.connectors.granola import (  # noqa: PLC0415
        GranolaKeyError,
...
def test_validate_key_rejects_unauthorized(status: int) -> None:
    ...
    with patch(
        "nova.connectors.granola.httpx.get",
...
@patch("nova.connectors.granola._granola_api_validate_key")
...
def test_handle_callback_invalid_key_does_not_overwrite_existing(connector) -> None:
    ...
    from nova.connectors.granola import GranolaKeyError  # noqa: PLC0415
