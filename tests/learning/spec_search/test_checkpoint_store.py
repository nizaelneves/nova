"""Tests for nova.learning.spec_search.checkpoint.store module."""
...
def _setup_isolated_repo_root(tmp_path: Path) -> Path:
    """Create a fake nova-home directory tree with config files for the
    CheckpointStore to track. Returns the root."""
    root = tmp_path / "nova_home"
...
class TestCheckpointStoreInit:
    ...
    def test_creates_repo_with_baseline_commit(self, tmp_path: Path) -> None:
        from nova.learning.spec_search.checkpoint.store import (
            CheckpointStore,
        )
    ...
    def test_init_refuses_inside_source_tree(
        ...
        bad_root = source_root / "fake_nova_home"
