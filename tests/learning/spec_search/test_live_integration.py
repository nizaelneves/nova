...
- TraceStore at ~/.nova/traces.db with some traces
"""
...
def real_trace_store():
    ...
    db_path = Path.home() / ".nova" / "traces.db"
    if not db_path.exists():
        pytest.skip("No traces.db found at ~/.nova/")
...
class TestDiagnosisRunnerLive:
    ...
    def test_diagnosis_produces_output(
        ...
        from nova.learning.spec_search.diagnose.runner import (
            DiagnosisRunner,
        )
...
class TestColdStartLive:
    ...
    def test_orchestrator_cold_start_with_no_feedback(
        ...
        from nova.learning.spec_search.gate.cold_start import (
