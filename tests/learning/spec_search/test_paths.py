"""Tests for nova.learning.spec_search.storage.paths module."""
...
class TestResolveSpecSearchRoot:
    ...
    def test_default_is_under_home(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from nova.learning.spec_search.storage import paths

        monkeypatch.delenv("NOVA_HOME", raising=False)
        result = paths.resolve_spec_search_root()
        assert result == Path.home() / ".nova" / "learning"

    def test_respects_nova_home_env_var(
        self,
        tmp_path: Path,
    ...
    def test_rejects_path_inside_source_tree(
        ...
        # Find the Nova source root by walking up from the paths module.
        source_root = paths._find_source_root()
        assert source_root is not None  # We must be running inside the repo.

        # Force NOVA_HOME to point inside the source tree.
