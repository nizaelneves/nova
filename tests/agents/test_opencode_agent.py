...
from nova.agents.opencode import (
    OpenCodeAgent,
...
class TestAvailability:
    def test_true(self, monkeypatch):
        monkeypatch.setattr(
            "nova.agents.opencode.shutil.which", lambda n: "/usr/bin/opencode"
        )
        assert is_opencode_available() is True

    def test_false(self, monkeypatch):
        monkeypatch.setattr("nova.agents.opencode.shutil.which", lambda n: None)
...
class TestRunParsing:
    def test_run_parses_message_and_tools(self, monkeypatch, tmp_path):
        ...
        # the model was addressed as nova/local-model
