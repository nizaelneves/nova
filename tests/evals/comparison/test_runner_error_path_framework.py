...
default of ``"nova"`` to silently mislabel data from foreign
...
from nova.evals.core.types import EvalRecord, RunConfig
...
class TestErrorPathFrameworkPropagation:
    def test_failed_task_keeps_backend_framework_name(self, tmp_path: Any) -> None:
        ...
        backend's framework_name, not the dataclass default 'nova'."""
        cfg = RunConfig(
            benchmark="mock",
            backend="hermes",
            model="mock-model",
        ...
        # NOT from the EvalResult dataclass default of "nova".
    ...
    def test_default_framework_when_backend_missing_attr(self, tmp_path: Any) -> None:
        ...
        a sensible default (the ABC's "nova"), via getattr fallback."""
        ...
        # the conservative "nova" tag rather than crashing.
        assert results[0].framework == "nova"
