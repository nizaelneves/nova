"""Tests for nova.evals.backends.external.hermes_agent.HermesBackend."""
...
from nova.evals.backends.external._subprocess_runner import SubprocessResult
from nova.evals.backends.external.hermes_agent import HermesBackend
from nova.evals.comparison.third_party import (
    CommitDriftError,
    ThirdPartyConfig,
    ThirdPartyEntry,
)
    ...
    return ThirdPartyConfig(
        entries={
            "hermes": ThirdPartyEntry(
                ...
                runner_script=(
                    "src/nova/evals/backends/external/_runners/hermes_runner.py"
                ),
                python_executable="",
            )
        }
...
class TestHermesBackend:
    def test_generate_full_builds_correct_subprocess_command(
        ...
        with (
            patch(
                "nova.evals.backends.external.hermes_agent.load_third_party_config",
                return_value=cfg,
            ),
            patch(
                "nova.evals.backends.external.hermes_agent.verify_commit_pin",
            ),
            patch(
                "nova.evals.backends.external.hermes_agent.run_one_shot",
            ) as mock_run,
        ):
