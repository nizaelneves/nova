"""Tests for the env-aware Nova home-directory resolver (issue #462).

Covers the single-root consolidation: ``$NOVA_HOME`` >
``$XDG_DATA_HOME/nova`` > ``~/.nova``, backward compatibility
(no env => exactly ``~/.nova``), and the source-tree rejection guard.
"""
...
class TestGetConfigDir:
    ...
    def test_default_when_unset_is_legacy_dir(
        ...
        # historical ~/.nova so existing installs are untouched.
        _clear_env(monkeypatch)
        assert paths.get_config_dir() == (Path.home() / ".nova").resolve()

    def test_respects_nova_home(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _clear_env(monkeypatch)
        custom = tmp_path / "oj"
        monkeypatch.setenv("NOVA_HOME", str(custom))
    ...
    def test_respects_xdg_data_home(
        ...
        # Single nested 'nova' dir under XDG_DATA_HOME.
        assert paths.get_config_dir() == (tmp_path / "nova").resolve()

    def test_nova_home_wins_over_xdg(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _clear_env(monkeypatch)
        oj = tmp_path / "oj_wins"
        monkeypatch.setenv("NOVA_HOME", str(oj))
        monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path / "xdg_loses"))
        assert paths.get_config_dir() == oj.resolve()

    def test_expands_user_in_nova_home(self, monkeypatch: pytest.MonkeyPatch) -> None:
        _clear_env(monkeypatch)
        monkeypatch.setenv("NOVA_HOME", "~/relocated-oj")
    ...
    def test_returns_absolute_path(
        ...
        _clear_env(monkeypatch)
        monkeypatch.setenv("NOVA_HOME", str(tmp_path / "rel"))
...
class TestDerivedDirs:
    ...
    def test_config_path(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        _clear_env(monkeypatch)
        monkeypatch.setenv("NOVA_HOME", str(tmp_path / "oj"))
    ...
    def test_cache_dir_under_xdg(
        ...
        _clear_env(monkeypatch)
        monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path))
        assert paths.get_cache_dir() == (tmp_path / "nova" / "cache").resolve()
...
class TestSourceTreeRejection:
    ...
    def test_rejects_path_inside_source_tree(
        ...
        monkeypatch.setenv("NOVA_HOME", str(source_root / "junk_dir"))
...
class TestLegacyConstantsHonorEnv:
    ...
    NOVA_HOME entirely. The constant is resolved once at import (the
    ...
    def test_dataclass_defaults_reflect_env(
        ...
        # instantiation time, not freeze ~/.nova at import.
        _clear_env(monkeypatch)
        from nova.core.config import SessionConfig, StorageConfig
    ...
    def test_downstream_consumer_honors_env(
        ...
        _clear_env(monkeypatch)
        from nova.core import credentials
    ...
    def test_constant_never_resolves_to_the_real_developer_home(self) -> None:
        ...
        first import of nova.core.config anywhere in the test
        session -- which happens via tests/conftest.py, before any test
        body runs. If conftest.py doesn't set NOVA_HOME before that
        import, this constant (and the ~45 modules that reference it)
        permanently binds to the developer's real ~/.nova for the
        ...
        real_home = (Path.home() / ".nova").resolve()
        assert DEFAULT_CONFIG_DIR != real_home
        assert "NOVA_CONFIG" not in os.environ
