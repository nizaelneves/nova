...
def _get_fact_store():
    ...
    from nova.memory.store import create_fact_store

    config = load_config()
    mem = config.memory
    return create_fact_store(
        getattr(mem, "backend", "local"),
        path=getattr(mem, "facts_path", "~/.nova/memory_facts.jsonl"),
...
def list_facts() -> None:
    ...
    if any_quarantined:
        console.print(
            ...
            "one and run [bold]nova memory trust <#>[/bold] to allow recall."
        )
...
def trust_fact(index: int) -> None:
    ...
    if not 1 <= index <= len(facts):
        console.print(
            ...
            f"{len(facts)} fact(s) stored — see [bold]nova memory list[/bold]."
        )
        raise SystemExit(1)
