...
from nova.cli._bootstrap import detect_cloud_keys
from nova.cli.model import find_model_spec, hf_download, ollama_pull
...
def init(
    ...
    """Detect hardware and generate ~/.nova/config.toml."""
    ...
    if DEFAULT_CONFIG_PATH.exists() and not force:
        ...
        if preset:
            console.print(
                ...
                f"[bold]nova init --preset {preset} --force[/bold] to confirm."
            )
        else:
    ...
    if preset:
        examples_dir = (
            Path(__file__).resolve().parents[2] / "configs" / "nova" / "examples"
        )
        # Also check installed package location
        if not examples_dir.exists():
            examples_dir = (
                Path(__file__).resolve().parents[3] / "configs" / "nova" / "examples"
            )
    ...
    # the canonical location that subsequent ``nova`` commands load.
