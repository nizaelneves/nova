...
def _get_config_path(path: str | None) -> Path:
    ...
    from nova.core.config import DEFAULT_CONFIG_PATH

    if path:
        return Path(path)
    return Path(os.environ.get("NOVA_CONFIG", DEFAULT_CONFIG_PATH))


def _show_hardware_info(console: Console, show_recommendations: bool = True) -> None:
    """Display detected hardware information."""
    from nova.core.config import detect_hardware, recommend_engine
...
@config.command("path")
def show_path() -> None:
    """Print the resolved Nova directories (home, config, cache).

    All Nova state lives under a single root, resolved in priority
    order: ``$NOVA_HOME`` > ``$XDG_DATA_HOME/nova`` >
    ``~/.nova``. Use this to confirm where your data is stored after
    setting an override.
    """
    from nova.core.paths import get_cache_dir, get_config_dir, get_config_path
...
def set_config(key: str, value: str) -> None:
    ...
    from nova.core.config import DEFAULT_CONFIG_DIR, validate_config_key
    ...
    config_path = Path(
        os.environ.get("NOVA_CONFIG", DEFAULT_CONFIG_DIR / "config.toml")
    )
    if config_path.exists():
