"""``nova doctor`` — run diagnostic checks on the Nova installation."""
...
from nova.core.config import DEFAULT_CONFIG_PATH, load_config
...
def _check_default_model(
    ...
    from nova.intelligence.model_catalog import resolve_model_id_for_engine
...
def _check_optional_deps() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    optional_packages = [
        ("fastapi", "nova[server]", "REST API server"),
        ("torch", "pip install torch", "SFT/GRPO training"),
        ("pynvml", "nova[gpu-metrics]", "NVIDIA energy monitoring"),
        ("amdsmi", "nova[energy-amd]", "AMD energy monitoring"),
        ("colbert", "nova[memory-colbert]", "ColBERT memory backend"),
        (
            "zeus_apple_silicon",
            "nova[energy-apple]",
            "Apple Silicon energy monitoring",
        ),
        ("apple_fm_sdk", "nova[afm]", "Apple Foundation Models (AFM 3)"),
    ]
...
def doctor(as_json: bool) -> None:
    """Run diagnostic checks on your Nova installation."""
    ...
    console = Console()
    console.print()
    console.print("[bold]Nova Doctor[/bold]")
    console.print()
