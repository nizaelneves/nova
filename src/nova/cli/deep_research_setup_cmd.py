...
def _instantiate_connector(connector_id: str, config: Dict[str, Any]) -> Any:
    ...
    if connector_id == "apple_notes":
        ...
        from nova.connectors.notion import NotionConnector

        return NotionConnector()
    elif connector_id == "granola":
        from nova.connectors.granola import GranolaConnector
...
def _launch_chat(store: KnowledgeStore, console: Console) -> None:
    ...
    from nova.connectors.retriever import TwoStageRetriever
    ...
    from nova.tools.knowledge_search import KnowledgeSearchTool
    from nova.tools.knowledge_sql import KnowledgeSQLTool
    from nova.tools.scan_chunks import ScanChunksTool
