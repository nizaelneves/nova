...
from nova.cli._banner import print_banner
from nova.cli._tool_names import resolve_tool_names
from nova.cli.hints import hint_no_engine
...
# below: a screenshot is sensitive and Nova is local-first, so sending
...
def _run_research(
    *,
    ...
    from nova.agents.research_loop import DEFAULT_PLANNER_MODEL, ResearchAgent
    from nova.connectors.embeddings import OllamaEmbedder
...
def _run_agent(
    ...
    if config.agent.context_from_memory:
        try:
            from nova.tools.storage.context import ContextConfig, inject_context
...
def ask(
    ...
    if capture_screen:
        try:
            from nova.cli._screen import capture_screen_to_temp
