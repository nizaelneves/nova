...
from nova.core.credentials import inject_credentials
...
def _resolve_server_cors_origins(configured: object) -> list[str]:
    ...
    raw = os.environ.get("NOVA_CORS_ORIGINS", "").strip()
    if raw:
...
def serve(
    ...
    # Tool credentials saved through the browser UI live in the Nova
    ...
    api_key = _os.environ.get("NOVA_API_KEY", "")
    if not api_key:
        try:
            import tomllib
    ...
    from nova.core.credentials import TOOL_CREDENTIALS, get_credential_status
    ...
    if channel_bridge is not None:
        try:
            from nova.server.channel_bridge import (
                ChannelBridge,
            )
            from nova.server.session_store import (
                SessionStore,
            )
    ...
    console.print(
        f"[green]Starting Nova API server[/green]\n"
    ...
    from nova.server.daemon import run_server
