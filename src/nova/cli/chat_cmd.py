...
from nova.cli._runtime_panel import runtime_cli_options
from nova.cli._tool_names import resolve_tool_names
from nova.cli._voice_chat import VOICE_EXIT, VoiceSession, read_voice_input, speak
from nova.core.config import load_config
from nova.core.events import EventBus
from nova.core.types import Message, Role
from nova.memory import publish_completed_exchange
...
def _safe_rich_label(value: object) -> str:
    ...
    from nova.cli._model_switch import sanitize_model_id
...
@click.option(
    ...
    default=False,
    help=(
        "Show the model list before chat. Bare ``nova`` already does this on a TTY "
        "unless NOVA_SKIP_MODEL_PICK=1."
    ),
)
...
@click.option(
    ...
    default=None,
    help=(
        "Named persona dir under ~/.nova/personas/<name>/ "
...
def chat(
    ...
    engine list first (bare ``nova`` does this on a TTY unless
    ``NOVA_SKIP_MODEL_PICK=1``).
    ...
    # Resolve engine
    from nova.engine import get_engine
    from nova.intelligence import register_builtin_models
        ...
        try:
            ...
            if AgentRegistry.contains(agent_key):
                ...
                if getattr(agent_cls, "accepts_tools", False):
                    ...
                    if tool_names_list:
                        import nova.tools  # noqa: F401 — trigger registration
                ...
                from nova.security.runtime import agent_security_kwargs
                ...
                from nova.security.runtime import wire_agent_security
    ...
    console.print(
        f"[green bold]Nova Chat[/green bold]\n"
    ...
    memory_service = None
    try:
        from nova.memory import build_memory_service
    ...
    if config.agent.context_from_memory:
        from nova.cli.ask import _get_memory_backend
    ...
    while True:
        ...
        if config.agent.context_from_memory:
            try:
                from nova.memory import load_configured_facts
