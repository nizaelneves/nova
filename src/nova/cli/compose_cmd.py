...
@click.group()
def compose() -> None:
    """Compose, run, benchmark, and deploy Nova configurations.
...
def compose_list(kind: Optional[str]) -> None:
    ...
    try:
        from nova.recipes.loader import discover_recipes

        recipes = discover_recipes(kind=kind)
        if not recipes:
            console.print("[dim]No compositions found.[/dim]")
            console.print(
                "[dim]Place TOML recipes in src/nova/recipes/data/ "
                "or ~/.nova/recipes/[/dim]"
            )
            return
...
def compose_show(name: str) -> None:
    ...
    console = Console(stderr=True)
    try:
        from nova.recipes.loader import resolve_recipe
...
def compose_stop(name: str) -> None:
    ...
    try:
        ...
        from nova.recipes.loader import _PROJECT_OPERATORS_DIR
