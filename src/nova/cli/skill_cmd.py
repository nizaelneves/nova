...
def install(query: str, with_scripts: bool, force: bool, url: str, yes_dangerous: bool):
    ...
    from nova.skills.importer import SkillImporter
    from nova.skills.parser import SkillParser
    from nova.skills.tool_translator import ToolTranslator
...
def remove(skill_name: str, yes: bool):
    ...
    Searches ``~/.nova/skills/`` and ``./skills`` for a directory whose
...
def discover(min_frequency: int, min_outcome: float, dry_run: bool) -> None:
    ...
    discovered skill manifests under ~/.nova/skills/discovered/."""
    console = Console()
    ...
    if dry_run:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="nova-discover-dryrun-"))
        try:
