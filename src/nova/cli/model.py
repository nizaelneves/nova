...
from nova.engine import discover_engines, discover_models
from nova.intelligence import merge_discovered_models, register_builtin_models
...
def _convert_mlx(hf_repo: str, output: str, mlx_4bit: bool, console: Console) -> bool:
    ...
    try:
        ...
        console.print(
            ...
            'Install it: [cyan]pip install "nova[inference-mlx]"[/cyan]'
        )
        return False
...
def convert(
    ...
    if engine in ("vllm", "sglang"):
        console.print(
            ...
            f"[cyan]Start with:[/cyan] nova host {shlex.quote(hf_repo)} "
            f"--backend {engine}"
        )
        return
    ...
    if engine == "mlx":
        ...
        hint = f"nova host {shlex.quote(str(artifact))} --backend mlx"
        ...
        if engine == "ollama":
            ...
            hint = f"nova chat --engine ollama --model {shlex.quote(ollama_name)}"
            title = "GGUF conversion and Ollama import complete"
        else:
            hint = f"nova host {shlex.quote(str(artifact))} --backend llamacpp"
