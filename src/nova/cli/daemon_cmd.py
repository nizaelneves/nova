...
from nova.core.config import DEFAULT_CONFIG_DIR, load_config
from nova.core.utils import process_alive, terminate_process
from nova.security.file_utils import secure_write_json, secure_write_text
...
def record_server_state(pid: int, host: str, port: int) -> None:
    """Register a running server so `nova status` can find it.

    Called by `nova serve` itself, so a server supervised by launchd/systemd
    (which never goes through `nova start`) is still reported as running.
    """
...
def start(
    ...
    """Start the Nova server as a background daemon."""
    ...
    cmd = [sys.executable, "-m", "nova.cli", "serve"]
    if host:
    ...
    console.print(
        f"[green]Nova server starting[/green] (PID {proc.pid})\n"
...
@daemon.command()
def stop() -> None:
    """Stop the running Nova server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
...
def restart(ctx: click.Context) -> None:
    """Restart the Nova server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
...
@daemon.command()
def status() -> None:
    """Show status of the Nova server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
