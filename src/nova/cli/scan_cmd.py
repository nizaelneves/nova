...
from nova.core.paths import get_config_dir, get_config_path
...
class PrivacyScanner:
    ...
    def check_icloud_sync(self) -> ScanResult:
        """Check whether ~/.nova is inside iCloud Drive sync scope."""
        try:
            ...
            if str(config_path).startswith(str(icloud_path)):
                return ScanResult(
                    name="iCloud Sync",
                    status="warn",
                    message="~/.nova may be synced to iCloud.",
                    platform="darwin",
                )
            ...
            return ScanResult(
                name="iCloud Sync",
                status="ok",
                message="~/.nova is not inside iCloud Drive.",
                platform="darwin",
            )
        except Exception:
            return ScanResult(
...
def _resolve_data_boundary_config_path() -> Path:
    env_config = os.environ.get("NOVA_CONFIG")
    if env_config:
...
def _load_data_boundary_config():
    ...
    if config_path is None or not config_path.exists():
        return NovaConfig(), root, False, "", root_error
    try:
        return load_config(config_path), root, True, "", root_error
    except Exception as exc:
        return NovaConfig(), root, False, f"{type(exc).__name__}: {exc}", root_error
...
def _render_results(results: List[ScanResult]) -> None:
    ...
    console = Console()
    console.print()
    console.print("[bold]Nova Security Scan[/bold]")
    console.print()
    ...
    if fail_count:
        console.print(
            ...
            "before storing sensitive data with Nova."
        )
        console.print()
    elif warn_count:
        console.print(
...
def _render_data_boundary_report(
    ...
    *,
    ...
    console = Console()
    console.print()
    console.print("[bold]Nova Data-Boundary Scan[/bold]")
