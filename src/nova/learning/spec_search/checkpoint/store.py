...
A thin wrapper over a local git repository at ``<nova_home>/.git``.
...
from nova.learning.spec_search.storage.paths import (
    ConfigurationError,
    _find_source_root,
)
...
class CheckpointStore:
    ...
    root :
        ...
        ``~/.nova/`` (or ``$NOVA_HOME``); for tests it's a
    ...
    def init(self) -> None:
        ...
        Refuses to initialize if ``self.root`` is inside the Nova source
        ...
        if source_root is not None:
            try:
                ...
                raise ConfigurationError(
                    ...
                    f"Nova source tree ({source_root}). Refusing to "
        ...
        if not (self._root / ".git").exists():
            self._git("init", "-q")
            self._git("config", "user.email", "spec-search@nova.local")
            self._git("config", "user.name", "Nova Spec Search")
        ...
        # Allow empty so init succeeds even on a brand-new nova home.
        self._git(
            "commit",
            "--allow-empty",
            "-q",
