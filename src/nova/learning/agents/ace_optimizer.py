...
The wrapper adapts Nova traces into ACE's
...
sidecar overlay under ``~/.nova/learning/ace/<task>/`` for the
...
from nova.core.config import ACEOptimizerConfig
...
class _TraceDataProcessor:
    """Adapter that exposes Nova traces in ACE's three-method API.
...
def _traces_to_samples(traces: List[Any]) -> List[Dict[str, Any]]:
    """Convert Nova trace records to ACE's sample dict shape."""
...
class ACEAgentOptimizer:
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_ACE:
            return {
                "status": "error",
                "reason": ("ace not installed (pip install 'nova[learning-ace]')"),
            }
