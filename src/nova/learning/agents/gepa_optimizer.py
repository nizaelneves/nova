...
Uses GEPA's adapter pattern to bridge Nova traces into GEPA's
...
from nova.core.config import GEPAOptimizerConfig
from nova.core.registry import LearningRegistry
from nova.learning._stubs import AgentLearningPolicy
...
class NovaGEPAAdapter:
    """Implements GEPA's adapter protocol for Nova agents.
...
class GEPAAgentOptimizer:
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_GEPA:
            return {
                "status": "error",
                "reason": ("gepa not installed (pip install 'nova[learning-gepa]')"),
            }

        agent_name = self.config.agent_filter or "default"
        adapter = NovaGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "NovaGEPAAdapter"]
