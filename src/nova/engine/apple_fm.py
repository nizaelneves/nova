...
Swift bindings at install time::
    ...
    nova ask --engine afm --model afm-3-core "..."
...
* There is no Private Cloud Compute path in the Python SDK, which suits
  Nova: off-device inference would make the on-device energy
...
from nova.engine._apple_fm_support import (
    MODEL_LABELS,
    SnapshotAccumulator,
...
from nova.engine._async_loop import AsyncLoopRunner
...
class AppleFMEngine(InferenceEngine):
    ...
    def _ensure_loop(self) -> AsyncLoopRunner:
        if self._loop is None:
            self._loop = AsyncLoopRunner(name="nova-afm")
        return self._loop
