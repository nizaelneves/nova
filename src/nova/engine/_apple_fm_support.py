...
a full Xcode on an Apple Silicon Mac, and Nova has no macOS runner.
...
def build_options_kwargs(params: Mapping[str, Any]) -> Dict[str, Any]:
    """Map Nova params onto ``fm.GenerationOptions`` fields.

    Only non-``None`` values are returned so the SDK's own defaults apply for
    anything the caller did not set. ``max_tokens`` is Nova's
