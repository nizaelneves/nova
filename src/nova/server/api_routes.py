...
def _execute_agent_admin_tool(request: Request, tool: Any, params: Dict[str, Any]):
    ...
    from nova.security.runtime import execute_secured_tool
...
async def list_agents(request: Request):
    ...
    try:
        from nova.tools.agent_tools import AgentListTool
    ...
    registered = []
    try:
        import nova.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from nova.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from nova.tools.agent_tools import AgentSpawnTool
...
async def kill_agent(agent_id: str, request: Request):
    ...
    try:
        from nova.tools.agent_tools import AgentKillTool
...
async def message_agent(agent_id: str, req: AgentMessageRequest, request: Request):
    ...
    try:
        from nova.tools.agent_tools import AgentSendTool
...
def memory_index(req: MemoryIndexRequest, request: Request):
    ...
    try:
        ...
        from nova.security.file_policy import is_sensitive_file
        from nova.tools.storage.ingest import ingest_path

        target = Path(req.path).expanduser().resolve()
        if not target.exists():
            raise HTTPException(status_code=404, detail=f"Path not found: {req.path}")

        # Sandbox: when workspace roots are configured via NOVA_WORKSPACE
        ...
        workspace = os.environ.get("NOVA_WORKSPACE", "").strip()
        if workspace:
            roots = [
...
def _telemetry_db_path(request: Request) -> Path:
    ...
    ``DEFAULT_CONFIG_DIR`` is fixed when :mod:`nova.core.config` is
    imported, so it cannot honor a later ``NOVA_HOME`` override.  The
...
async def list_skills(request: Request):
    ...
    try:
        from nova.core.registry import SkillRegistry
...
async def install_skill(request: Request):
    ...
    return {
        ...
        "message": "Use TOML files in ~/.nova/skills/",
    }
...
async def prometheus_metrics(request: Request):
    ...
    try:
        ...
        lines = [
            "# HELP nova_requests_total Total requests processed",
            "# TYPE nova_requests_total counter",
            f"nova_requests_total {stats.total_calls}",
            "# HELP nova_tokens_total Total tokens generated",
            "# TYPE nova_tokens_total counter",
            f"nova_tokens_total {stats.total_tokens}",
            "# HELP nova_latency_avg_ms Average latency in milliseconds",
            "# TYPE nova_latency_avg_ms gauge",
            f"nova_latency_avg_ms {avg_latency_ms}",
        ]
        from starlette.responses import PlainTextResponse
...
def _record_ws_trace(
    ...
    *,
    ...
    if trace_store is None or not result:
        return
    from nova.traces.collector import record_response_trace
...
async def websocket_chat_stream(websocket: WebSocket):
    ...
    """
    from nova.server.auth_middleware import authenticate_websocket
...
async def learning_stats(request: Request):
    ...
    # Skill discovery
    try:
        from nova.learning.agents.skill_discovery import SkillDiscovery
...
def include_all_routes(app) -> None:
    ...
    from nova.server.approval_routes import (
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from nova.server.agent_manager_routes import (  # noqa: PLC0415
    ...
    # singleton is a *different* bus that nothing in `nova serve` ever
    # publishes to, so events silently never reached this endpoint.
    try:
        from nova.core.events import get_event_bus
