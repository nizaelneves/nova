...
from nova.agents.tool_resolver import (
...
from nova.server.model_capabilities import is_embed_only_model
...
logger = logging.getLogger("nova.server.agent_manager")
...
def _make_lightweight_system(
    ...
    """
    try:
        from nova.engine._discovery import get_engine
        ...
        try:
            from nova.telemetry.instrumented_engine import (
                InstrumentedEngine,
            )
...
def build_tools_list() -> List[Dict[str, Any]]:
    ...
    from nova.core.registry import ChannelRegistry, ToolRegistry
...
def _get_mcp_tools_locked(
    ...
    from nova.core.config import load_config, resolve_mcp_servers
    ...
    from nova.mcp.transport import StdioTransport, StreamableHTTPTransport
    from nova.tools.mcp_adapter import MCPToolProvider
    ...
    if agent_type == "deep_research":
        ...
        if dr_tools:
            ...
            async def generate_deep_research():
                ...
                from nova.agents.deep_research import DeepResearchAgent
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from nova.core.types import ToolCall as MsgToolCall
...
def create_agent_manager_router(
    ...
    async def save_tool_credentials(tool_name: str, request: Request):
        from nova.core.credentials import save_credential
    ...
    def remove_tool_credential(tool_name: str, key: str):
        from nova.core.credentials import delete_credential
    ...
    def credential_status(tool_name: str):
        from nova.core.credentials import get_credential_status
