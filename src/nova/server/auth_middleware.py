"""API key authentication middleware for the Nova server."""
...
_WS_AUTH_PROTOCOL = "nova.auth.v1"
_WS_KEY_PROTOCOL_PREFIX = "nova.key.b64url."
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, api_key: str = "") -> None:  # noqa: ANN001
        super().__init__(app)
        self._api_key = api_key or os.environ.get("NOVA_API_KEY", "")
...
def check_bind_safety(host: str, *, api_key: str) -> None:
    ...
    *api_key* is empty.
    ...
    if not is_loop and not api_key:
        logger.error(
            "Binding to %s requires NOVA_API_KEY to be set. "
...
def authenticate_websocket(
    ...
    clients, which cannot set that header, offer ``nova.auth.v1`` plus a
