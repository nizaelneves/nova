"""FastAPI application factory for the Nova API server."""
...
from nova.server.comparison import comparison_router
    ...
    *,
    ...
    if security_enabled:
        ...
        if any(
            ...
            # enforcement as ``nova serve``. Explicitly injected primitives
    ...
    app = FastAPI(
        title="Nova API",
        description="OpenAI-compatible API server for Nova",
        version="0.1.0",
    )
    ...
    # NOVA_CORS_ORIGINS (comma-separated). On an exposed server this
    ...
    _env_origins = os.environ.get("NOVA_CORS_ORIGINS", "").strip()
    ...
    try:
        from nova.server.middleware import create_security_middleware
    ...
    if api_key:
        try:
            from nova.server.auth_middleware import AuthMiddleware
    ...
    if webhook_config:
        try:
            from nova.server.webhook_routes import (
