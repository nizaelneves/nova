...
HTTP API, configured to use Nova's local engine through an
OpenAI-compatible provider. This keeps coding-agent work local-first: opencode
handles the agentic loop / tools, Nova supplies the model.
...
def _derive_openai_base_url(engine: Any) -> str:
    """Best-effort OpenAI-compatible base URL for an Nova engine.
...
def _extract_tool_results(parts: List[dict]) -> List[ToolResult]:
    """Map opencode ``tool`` parts to Nova ToolResults (best-effort)."""
...
class OpenCodeAgent(BaseAgent):
    ...
    inference runs on Nova's selected local model. ``agent`` selects
    ...
    def _ensure_server(self) -> str:
        ...
        self._config_dir = tempfile.mkdtemp(prefix="nova-opencode-")
