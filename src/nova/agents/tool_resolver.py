...
def ensure_registries_populated() -> None:
    ...
    try:
        import nova.channels  # noqa: F401
    except Exception:
        pass
    ...
    browser_modules = ("nova.tools.browser", "nova.tools.browser_axtree")
    ...
    if not ChannelRegistry.keys():
        for module_name in list(sys.modules):
            if module_name.startswith("nova.channels.") and not module_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not ToolRegistry.keys():
        for module_name in list(sys.modules):
            if (
                module_name.startswith("nova.tools.")
