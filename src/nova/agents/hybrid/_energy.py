...
* Samples **all visible NVIDIA GPUs on the host where the runner runs**.
  ...
  ``NOVA_HYBRID_ENERGY=0`` to silence the warning.
...
class EnergyCollector:
    ...
    def __init__(self, sample_hz: float = 2.0) -> None:
        ...
        self._t0: float = 0.0
        self._t1: float = 0.0
        self._enabled = os.environ.get("NOVA_HYBRID_ENERGY", "1") != "0"
        self._pynvml = None
        self._handles: list = []
    ...
    def __enter__(self) -> "EnergyCollector":
        ...
        try:
            ...
            _log_once(
                ...
                "energy_j_total will be 0. Set NOVA_HYBRID_ENERGY=0 to silence."
            )
            self._pynvml = None
            return self
