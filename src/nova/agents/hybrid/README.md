...
harness — each is registered as a standard Nova agent so the rest
of the platform (SDK, CLI, distillation, evals) can use them like any
other agent. Results live under ``$NOVA_HYBRID_EXPERIMENTS_DIR``
(defaults to ``~/.nova/experiments/hybrid/``).
...
Items in parentheses are what the *paper* trains. These Nova ports
...
`src/nova/evals/scorers/swebench_harness.py` (next to the existing
structural scorer).
...
# 3. Run a smoke cell
.venv/bin/python -m nova.agents.hybrid.runner \
    --cell minions-gaia-qwen27b-opus-3
```

Outputs land in
`$NOVA_HYBRID_EXPERIMENTS_DIR/runs/<cell>/{results.jsonl,summary.json,config.json,logs/}`
(defaults to `~/.nova-hybrid/experiments/`). The schema matches the
...
```bash
src/nova/agents/hybrid/scripts/new_experiment.sh \
...
(`~/.nova/experiments/hybrid/docs/results.md`) at full N —
...
these Nova cells should reproduce the harness numbers within
