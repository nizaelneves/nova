...
::
    ...
    python -m nova.agents.hybrid.runner --cell minions-gaia-qwen27b-opus-3

Reads a cell definition from ``registry/<method>.toml`` (bundled with this
package or pointed at by ``NOVA_HYBRID_REGISTRY_DIR``), constructs
the registered agent, loads bench tasks via Nova's existing dataset
...
existing rescore / dashboard scripts can read Nova cells without
modification.
"""
...
from nova.agents._stubs import AgentContext, AgentResult
from nova.agents.hybrid._energy import EnergyCollector
from nova.agents.hybrid._prompts import format_prompt as _format_prompt
...
# ``NOVA_HYBRID_TASK_TIMEOUT_S`` (0 / negative disables).
DEFAULT_TASK_TIMEOUT_S = float(
    os.environ.get("NOVA_HYBRID_TASK_TIMEOUT_S", "1800") or 1800
)
...
def load_registry(registry_dir: Optional[Path] = None) -> Dict[str, Dict[str, Any]]:
    ...
    env_override = os.environ.get("NOVA_HYBRID_REGISTRY_DIR")
    if env_override:
...
def _load_gaia_tasks(n: Optional[int]) -> List[Dict[str, Any]]:
    ...
    for rec in ds.iter_records():
        ...
        # Nova prefixes with `gaia-`) so subsets keyed by the upstream
        # id round-trip.
        md = rec.metadata or {}
...
def _get_gaia_scorer():
    ...
    ``NOVA_GAIA_JUDGE_MODEL``); the judge backend is the ``cloud``
    engine, so ``NOVA_CONFIG`` needs a ``[engine.cloud]`` section.
    """
    global _GAIA_SCORER
    if _GAIA_SCORER is None:
        with _GAIA_SCORER_LOCK:
            if _GAIA_SCORER is None:
                from nova.evals.backends.nova_direct import (
                    NovaDirectBackend,
                )
                from nova.evals.scorers.gaia_exact import GAIAScorer

                judge_model = os.environ.get(
                    "NOVA_GAIA_JUDGE_MODEL", "gpt-5-mini-2025-08-07"
                )
                try:
...
def _score_gaia(task: Dict[str, Any], answer: str) -> Dict[str, Any]:
    ...
    Uses the shared Nova :class:`GAIAScorer`. The previous version
...
def _score_swebench(
    ...
    """
    from nova.evals.core.types import EvalRecord
    from nova.evals.scorers.swebench_harness import (
...
def _build_agent(cell: Dict[str, Any]):
    ...
    import nova.agents  # noqa: F401 — populate registry
...
def _run_cell_locked(
    ...
    *,
    ...
    # (env ``NOVA_HYBRID_TASK_TIMEOUT_S``, 1800s) applies. 0 disables.
    mcfg = cell.get("method_cfg") or {}
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="python -m nova.agents.hybrid.runner",
