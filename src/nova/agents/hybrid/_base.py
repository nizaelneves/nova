...
Why not just use Nova's :class:`InferenceEngine` for both? Two reasons:
...
from nova.agents.hybrid._openai_retry import patch_openai_globally
from nova.agents.hybrid._prices import (
    NO_TEMP_PREFIXES,
    is_gpt5_family,
...
def tavily_search_context(
    ...
    *,
    ...
    """Run Nova WebSearchTool and return accounting-friendly metadata."""
...
class LocalCloudAgent(BaseAgent):
    ...
    def _call_openrouter(
        ...
        *,
        ...
        pass the Nova-engine-style ``"openrouter/<provider>/<model>"``
        prefix (see ``src/nova/engine/cloud.py``) — we strip it here.
