...
both forced by the Nova environment and documented inline:
...
* in-tool correctness check — the original ``answer`` tool LLM-judges the
  prediction against the gold answer inside ``call_tool``. Nova
...
def run_answer(
    ...
    *,
    ...
    in-tool LLM correctness check is dropped — Nova scores
    downstream.
    """
...
def run_search(
    ...
    *,
    ...
    Nova substitution for the missing FAISS wiki index).
    """
    prompt = (
        context_str.strip()
        + "\n\n"
