"""Load system prompt and few-shot overrides from $NOVA_HOME.
...
- System prompts: ``$NOVA_HOME/agents/{name}/system_prompt.md``
- Few-shot exemplars: ``$NOVA_HOME/agents/{name}/few_shot.json``
...
def _nova_home() -> Path:
    """Resolve the Nova root, honoring NOVA_HOME / XDG_DATA_HOME."""
...
def load_system_prompt_override(agent_name: str) -> str | None:
    ...
    Looks for ``$NOVA_HOME/agents/<agent_name>/system_prompt.md``.
    ``NOVA_HOME`` defaults to ``~/.nova`` when unset.
    """
    home = _nova_home()
...
def load_few_shot_exemplars(
    ...
    Looks for ``$NOVA_HOME/agents/<agent_name>/few_shot.json``.
