...
class OAuthProvider:
    ...
    # Filenames in ~/.nova/connectors/ to save tokens to
...
def get_client_credentials(
    ...
    Checks credential files in ``~/.nova/connectors/`` and falls
    back to environment variables ``NOVA_{NAME}_CLIENT_ID`` and
    ``NOVA_{NAME}_CLIENT_SECRET``.
    """
    ...
    prefix = f"NOVA_{provider.name.upper()}"
        ...
        def do_GET(self) -> None:  # noqa: N802 — required override name
            ...
            if "code" in params:
                ...
                self.wfile.write(
                    ...
                    b"<p>You can close this tab and return to Nova.</p>"
                    b"</body></html>"
                )
