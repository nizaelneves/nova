"""Application data-boundary diagnostics for Nova.
...
def build_data_boundary_report(
    ...
    *,
    ...
    root_label = "<unresolved-nova-home>"
    if root is not None:
        try:
    ...
    if root_error:
        ...
        builder.add(
            ...
            status="fail",
            title="Nova home directory could not be resolved",
            potential_data_path="runtime state root -> local store checks",
            evidence=(
                f"{error_type} while resolving Nova home; path details were redacted"
            ),
            recommendation=(
                "Fix NOVA_HOME or XDG_DATA_HOME before relying on "
    ...
    if config_error:
        builder.add(
            ...
            status="fail",
            title="Nova config could not be loaded",
...
def _knowledge_store_exists(root: Path) -> bool:
    """True when knowledge.db exists under the Nova home (metadata only)."""
...
def _web_search_destination() -> str:
    ...
    """
    engine = (os.environ.get("NOVA_WEB_SEARCH_ENGINE") or "auto").strip().lower()
...
def _audit_server_exposure(config: Any, builder: _FindingBuilder) -> None:
    ...
    if not _is_loopback_host(host):
        ...
        builder.add(
            ...
            potential_data_path="Nova HTTP server -> local network interfaces",
    ...
    if a2a_enabled and not a2a_token:
        builder.add(
            ...
            potential_data_path="inbound A2A requests -> Nova agent runtime",
...
def _audit_channel_settings(config: Any, builder: _FindingBuilder) -> None:
    ...
    for dotted_path, label, service in configured_secrets:
        ...
        builder.add(
            ...
            potential_data_path=f"Nova channel messages -> {service}",
...
def _derive_verdict(findings: Iterable[DataBoundaryFinding]) -> str:
    ...
    if "config-root-error" in finding_ids:
        return "Nova home must be fixed before full data-boundary review"
