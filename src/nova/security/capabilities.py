...
_SAFE_BUILTIN_PROVENANCE = {
    "calculator": ("nova.tools.calculator", "CalculatorTool"),
    "think": ("nova.tools.think", "ThinkTool"),
}
...
def canonical_tool_capabilities(tool: Any) -> List[str]:
    ...
    is_builtin = (
        module == "nova.tools"
        or module.startswith("nova.tools.")
        or module == "nova.scheduler.tools"
    )
    if module == "nova.tools.mcp_adapter":
