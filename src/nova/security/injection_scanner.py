...
from nova.security.types import ScanFinding, ThreatLevel

# Longest ``matched_text`` retained on a finding.
...
class InjectionScanner:
    ...
    def scan(self, text: str) -> InjectionScanResult:
        """Scan text for injection patterns using ``_INJECTION_PATTERNS``.

        ``matched_text`` is capped at 100 characters and a clean result reports
        ``ThreatLevel.LOW``.
        """
        findings: List[ScanFinding] = []
        highest = -1
