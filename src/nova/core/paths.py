"""Central, env-aware resolution of Nova' home directory.

Nova keeps all of its runtime state (config, databases, caches, logs,
...
1. ``$NOVA_HOME`` — explicit override.
2. ``$XDG_DATA_HOME/nova`` — when ``$XDG_DATA_HOME`` is set, follow the
   XDG Base Directory spec by nesting a single ``nova`` directory under
...
3. ``~/.nova`` — the historical default. With no env vars set, the
...
override. Modules that previously hardcoded ``Path.home() / ".nova"``
should call :func:`get_config_dir` (or :func:`get_data_dir` /
:func:`get_cache_dir`) instead.

Defense in depth: the resolved root must never live inside the Nova
source tree (a misconfigured ``$NOVA_HOME`` pointing at the repo would
...
_DEFAULT_DIR_NAME = ".nova"
_XDG_SUBDIR_NAME = "nova"
...
def _find_source_root() -> Path | None:
    ...
    Returns the directory containing the Nova ``pyproject.toml`` (the one
    whose ``name = "nova"``), or ``None`` when running from an installed
...
def _reject_source_tree(path: Path) -> Path:
    """Raise if ``path`` resolves inside the Nova source tree."""
    ...
    if source_root is not None:
        try:
            ...
            raise ConfigurationError(
                f"Nova home ({path}) is inside the source tree "
                f"({source_root}). Nova refuses to write runtime state "
                "inside its own repo. Set NOVA_HOME (or XDG_DATA_HOME) "
...
def get_config_dir() -> Path:
    """Resolve Nova' single root directory, honoring env overrides.

    Precedence: ``$NOVA_HOME`` > ``$XDG_DATA_HOME/nova`` >
    ``~/.nova``. The result is always absolute and is rejected if it
    falls inside the Nova source tree.
    """
    env_home = os.environ.get("NOVA_HOME")
    if env_home:
...
def get_config_path() -> Path:
    """Resolve the path to ``config.toml`` under the Nova root."""
...
def get_cache_dir() -> Path:
    ...
    Lives at ``<root>/cache`` so caches stay inside the single Nova
