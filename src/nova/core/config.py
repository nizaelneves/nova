...
User configuration lives at ``~/.nova/config.toml``.  ``load_config()``
...
# once at import via the env-aware resolver in ``nova.core.paths`` (the
# install-script model: ``NOVA_HOME`` / ``XDG_DATA_HOME`` are set before
...
class ACEOptimizerConfig:
    ...
    Install via ``pip install -e nova[learning-ace]`` once the
    ...
    # most Nova users have credentials for.
    ...
    # Empty string defaults to ``~/.nova/learning/ace/<task>/``.
    save_dir: str = ""
    task_name: str = "nova"
...
class StorageConfig:
    ...
    # the background and persists them across sessions (see nova.memory).
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        "You are Nova, a helpful AI assistant running locally on the "
        ...
        "who or what you are, identify yourself as Nova. Respond "
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.nova/whatsapp_auth
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.nova/scheduler.db
...
@dataclass
class NovaConfig:
    """Top-level configuration for Nova."""
...
def validate_config_key(dotted_key: str) -> type:
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            ...
            import nova.core.config as _cfg_mod
...
def load_config(path: Optional[Path] = None) -> NovaConfig:
    ...
    ----------
    path:
        Explicit config file. If not set, uses ``NOVA_CONFIG`` when set,
        otherwise ``~/.nova/config.toml``.
    """
    _ensure_config_dir()
    hw = detect_hardware()
    ...
    elif os.environ.get("NOVA_CONFIG"):
        config_path = Path(os.environ["NOVA_CONFIG"]).expanduser().resolve()
    else:
...
def generate_default_toml(
    ...
    """Render a commented TOML string suitable for ``~/.nova/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine)
    gpu_line = ""
    if hw.gpu:
...
# background and persists them. Starts/stops with `nova serve` and
# `nova chat`; manage stored facts with `nova memory list` / `clear`.
[memory]
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.nova/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.nova/whatsapp_auth
# assistant_name = "Nova"
