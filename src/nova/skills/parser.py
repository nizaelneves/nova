...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.nova.* via FIELD_MAPPING.
...
# dataclass attribute.  When target_kind is "nova_meta" the value is
# stored under manifest.metadata["nova"][attr].
FIELD_MAPPING: Dict[str, tuple[str, str]] = {
    ...
    "platforms": ("nova_meta", "platforms"),
    "prerequisites": ("nova_meta", "prerequisites"),
}
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.nova.original_frontmatter,
        and merges metadata.nova.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize nova namespace
        oj_meta = dict(raw_metadata.get("nova") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.nova.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.nova.* into canonical fields (these override
        # top-level mappings since they are explicit Nova-namespaced).
        for key in (
            "version",
            "author",
            "tags",
        ...
        # Preserve unmapped fields under metadata.nova.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the nova metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["nova"] = oj_meta
