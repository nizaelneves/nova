...
from nova.skills.dependency import DependencyCycleError, validate_dependencies
from nova.skills.executor import SkillExecutor, SkillResult
...
class SkillManager:
    ...
    def __init__(
        ...
        *,
        ...
        if overlay_dir is None:
            ...
            # ~/.nova/learning/skills/ if config can't be loaded.
            try:
    ...
    def _load_overlays(self) -> None:
        ...
        examples under ``manifest.metadata.nova.few_shot``.

        Bad overlays are silently ignored — they should not break discovery.
        """
        from nova.skills.overlay import SkillOverlayLoader
        ...
        for name, manifest in self._skills.items():
            ...
            if overlay.few_shot:
                ...
                oj = dict(new_metadata.get("nova", {}) or {})
                oj["few_shot"] = list(overlay.few_shot)
                new_metadata["nova"] = oj
    ...
    def get_few_shot_examples(self) -> List[str]:
        ...
        Pulls from ``manifest.metadata.nova.few_shot`` for every
        ...
        for name, manifest in self._skills.items():
            if manifest.disable_model_invocation:
                continue
            oj = manifest.metadata.get("nova", {}) if manifest.metadata else {}
    ...
    def discover_from_traces(
        ...
        *,
        ...
        ``~/.nova/skills/discovered/``).  Returns a list of dicts with
    ...
    def _serialize_discovered_skill(name: str, skill: Any) -> str:
        ...
        lines.append('author = "nova (auto-discovered)"')
    ...
    def find_installed_paths(
        ...
        for root in roots:
            ...
            for candidate in root.rglob("*"):
                ...
                try:
                    from nova.skills.loader import load_skill_directory
