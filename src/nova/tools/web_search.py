...
Engine selection is explicit (``engine=`` or ``NOVA_WEB_SEARCH_ENGINE``)
...
ENGINE_ENV = "NOVA_WEB_SEARCH_ENGINE"
ENGINES = ("auto", "youcom", "tavily", "duckduckgo")

# Identifies Nova to You.com. The keyless tier carries no API key, so the
# User-Agent is the only attribution signal; sent to You.com hosts only.
YOUCOM_USER_AGENT = f"nova/{__version__} youdotcom-integration/open-jarvis-openjarvis"
...
YOUCOM_PLATFORM_URL = (
    ...
    "?utm_source=nova-nova&utm_medium=oss_integration"
...
class WebSearchTool(BaseTool):
    ...
    def __init__(
        ...
        *,
        ...
        comes from ``NOVA_WEB_SEARCH_ENGINE`` and defaults to ``"auto"``.
    ...
    def _fetch_url(url: str, max_chars: int = 6000) -> str:
        ...
        resp = httpx.get(
            ...
            timeout=30.0,
            headers={
                "User-Agent": "Mozilla/5.0 (compatible; Nova/1.0; +https://github.com/openjarvis)"
            },
        )
