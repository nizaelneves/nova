...
from nova.core.events import EventBus, get_event_bus
...
from nova.tools._stubs import BaseTool, ToolExecutor
...
class SystemBuilder:
    ...
    def _build(self) -> NovaSystem:
        ...
        if telemetry_enabled and config.telemetry.gpu_metrics:
            try:
                from nova.telemetry.energy_monitor import (
    ...
    def _resolve_memory(self, config):
        try:
            import nova.tools.storage  # noqa: F401 -- trigger registration
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            import nova.channels  # noqa: F401 -- trigger registration
            from nova.core.registry import ChannelRegistry
            from nova.system._channel_kwargs import build_channel_kwargs
    ...
    def _resolve_tools(
        ...
        *,
        ...
        if config.tools.mcp.enabled and config.tools.mcp.servers:
            try:
                from nova.core.config import resolve_mcp_servers
    ...
    def _setup_sandbox(self, config):
        ...
        if not sandbox_enabled:
            return None
        try:
            from nova.sandbox.runner import ContainerRunner
    ...
    def _setup_learning_orchestrator(config: NovaConfig):
        ...
        try:
            from nova.core.config import DEFAULT_CONFIG_DIR
            from nova.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from nova.learning.training.lora import LoRATrainingConfig
            from nova.traces.store import TraceStore
